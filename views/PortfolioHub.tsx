...
const PortfolioHubView: React.FC = () => {
  ...
  const handleCreate = async () => {
    const newItem = { title: 'Untitled Note', original_text: '' };
    setIsSaving(true);
    try {
      const { data, error } = await supabase
        .from('portfolio_hub')
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await supabase.from('portfolio_hub').delete().eq('id', id);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-fadeIn">
      <div className="lg:col-span-4 space-y-6">
        <div className="flex items-center justify-between mb-4">
           <div>
              <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">Ledger Entries</h2>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Portfolio Knowledge Base</p>
           </div>
           <Button onClick={handleCreate} disabled={isSaving} icon={Plus} className="h-10 px-4 text-[10px] uppercase tracking-widest">New Entry</Button>
        </div>
        ...
        <div className="space-y-2 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
           {isLoading ? (
             ...
             >
                <div className="flex-1 min-w-0 pr-4">
                   ...
                   <p className="text-[10px] text-slate-400 font-medium truncate italic">{item.original_text || 'No description yet...'}</p>
                </div>
           ...
           {!isLoading && filteredItems.length === 0 && (
             <div className="p-12 text-center text-slate-400 font-bold uppercase text-[10px] italic bg-white rounded-2xl border border-dashed">No entries found</div>
           )}
        </div>
      </div>
      ...
      <div className="lg:col-span-8">
        {selectedId ? (
            ...
            <Card className="p-8" accentColor="indigo">
               <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Editor Session</h3>
                        <p className="text-lg font-black text-slate-900 tracking-tight uppercase">Document Ledger: {editor.title || 'Draft'}</p>
                     </div>
                  </div>
                  <div className="flex gap-2">
                     ...
                     <Button onClick={handleSave} disabled={isSaving} icon={Save} variant="primary" className="h-10 px-6 text-[10px] uppercase tracking-widest">
                        {isSaving ? <Loader2 size={16} className="animate-spin" /> : 'Sync to Cloud'}
                     </Button>
                  </div>
               </div>
               ...
               <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Portfolio Entry Title</label>
                    <input 
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Original Context / Description</label>
                    <textarea 
                      ...
                      placeholder="Document original production insights, material specs, or strategy notes here..."
                    />
                  </div>
               </div>
            </Card>
            ...
            <div className="grid grid-cols-2 gap-6">
               <Card className="p-6 bg-slate-900 text-white" accentColor="cyan">
                  <div className="flex items-center gap-3 mb-4">
                     ...
                     <p className="text-[10px] font-black uppercase tracking-widest text-cyan-400">Ledger Metrics</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                     <div>
                        <p className="text-[9px] font-black text-slate-500 uppercase">Char Count</p>
               ...
               <Card className="p-6" accentColor="slate">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <p className="text-xs font-bold text-slate-700 uppercase">Cloud Integrity Verified</p>
                  </div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">Last Modified: {selectedId ? new Date().toLocaleTimeString() : 'N/A'}</p>
               </Card>
            </div>
          </div>
        ) : (
          <div className="h-full flex flex-col items-center justify-center p-20 text-center space-y-6">
             ...
             <div>
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Access Financial Ledger</h3>
                <p className="text-slate-500 text-sm font-medium mt-2 max-w-xs">Select an existing document from the sidebar or initialize a new entry to document your portfolio logic.</p>
             </div>
             <Button onClick={handleCreate} icon={Plus} variant="primary" className="px-10 py-4 text-[10px] uppercase tracking-[0.2em] shadow-2xl">Start New Ledger</Button>
          </div>
        )}
      </div>
    </div>
